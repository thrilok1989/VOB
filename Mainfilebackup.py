...
# -----------------------
#  Dhan API - FIXED with correct indices endpoint
# -----------------------
@st.cache_data(ttl=5)
def get_nifty_spot_price():
    """Fetch NIFTY spot from Dhan indices API"""
    try:
        # Use indices API endpoint instead of marketfeed/ltp
        ...
        if response.status_code == 200:
            ...
            if data.get("data") and len(data["data"]) > 0:
                # Get the latest close price from intraday data
                latest = data["data"][-1]
                return float(latest.get("close", 0.0))
        
        # Fallback: Try to get from historical daily data
        url_daily = f"{DHAN_BASE_URL}/v2/charts/historical"
        payload_daily = {
            "securityId": "13",
            "exchangeSegment": "IDX_I",
            "instrument": "INDEX",
        ...
        response_daily = requests.post(url_daily, json=payload_daily, headers=headers, timeout=10)
        
        if response_daily.status_code == 200:
            data_daily = response_daily.json()
            if data_daily.get("data") and len(data_daily["data"]) > 0:
                latest_daily = data_daily["data"][-1]
                return float(latest_daily.get("close", 0.0))
        
        return 0.0
        
    except Exception as e:
        st.warning(f"Dhan spot fetch failed: {e}")
        return 0.0
...
with col1:
    ...
    if spot == 0.0:
        st.error("Unable to fetch NIFTY spot. Check credentials or try again.")
        st.stop()

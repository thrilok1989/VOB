...
# =============================================
# ENHANCED DATA FETCHER WITH FALLBACKS
# =============================================
...
class RobustDataFetcher:
    ...
    def fetch_nifty_data_with_fallback(self, symbol: str = "^NSEI", period: str = "7d", interval: str = "5m") -> pd.DataFrame:
        ...
        # Method 2: Try alternate symbols
        alternate_symbols = ["^NSEI", "NSEI", "NIFTY.NS", "NSE:NIFTY"]
    ...
    def _process_yahoo_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in required_cols:
            if col not in df.columns:
                df[col] = 10000 if col != 'volume' else 1000000
...
class DemoOptionsAnalyzer:
    """Simplified options analyzer for demonstration"""
    ...
    def get_overall_market_bias(self, force_refresh: bool = False) -> List[Dict[str, Any]]:
        ...
        for instrument in instruments:
            ...
            results.append({
                ...
                'pcr_change': np.random.uniform(0.7, 1.5),
                'total_ce_oi': np.random.randint(1000000, 5000000),
                'total_pe_oi': np.random.randint(1000000, 5000000),
                'total_ce_change': np.random.randint(-100000, 100000),
                'total_pe_change': np.random.randint(-100000, 100000),
                'detailed_atm_bias': {
...
# =============================================
# ENHANCED NIFTY APP WITH ROBUST DATA FETCHING
# =============================================
...
class EnhancedNiftyApp:
    ...
    def create_enhanced_chart(self, df: pd.DataFrame) -> Optional[go.Figure]:
        """Create enhanced price chart with volume order blocks"""
        if df.empty:
            return None
        ...
        for block in bullish_blocks:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[block['price_level'] - 50, block['price_level'] + 50],
                    mode='lines',
            ...
            if not df.empty:
                ...
                if show_volume_blocks:
                    ...
                    with col1:
                        ...
                        for block in st.session_state.volume_blocks['bullish'][-3:]:
                            st.write(f"- ₹{block['price_level']:.2f} (Vol: {block['volume']:,})")
                    
                    with col2:
                        st.write("**Bearish Volume Blocks**")
                        for block in st.session_state.volume_blocks['bearish'][-3:]:
                            st.write(f"- ₹{block['price_level']:.2f} (Vol: {block['volume']:,})")

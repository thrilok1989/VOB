...
class AnalysisState:
    ...
    def update_technical(self, result: Dict):
...
class TelegramNotifier:
    ...
    def send_bias_alert(self, technical_bias: str, options_bias: str, overall_bias: str, score: float):
        """Send comprehensive bias alert"""
        components = [technical_bias, options_bias]
...
def calculate_overall_nifty_bias():
    ...
    # Technical Analysis Bias (60% weight)
    ...
    if tech_result and tech_result.get('success'):
        ...
        bias_weights.append(0.60)
    
    # Enhanced Options Chain Bias (40% weight)
...
# Enhanced tabs (REMOVED PRICE ACTION TAB)
tabs = st.tabs([
    "Overall Bias", "Bias Summary", "Option Chain", "Bias Tabulation"
])
    ...
    if not options_data:
        ...
        for instrument_idx, instrument_data in enumerate(options_data):
            with st.expander(f"🏦 {instrument_data['instrument']} - CORRECTED Institutional Analysis", expanded=True):
                ...
                # INSTITUTIONAL OI PATTERNS ANALYSIS
                if 'institutional_analysis' in instrument_data:
                    ...
                    # Pattern breakdown with proper interpretation
                    col1, col2, col3 = st.columns(3)
                    with col1:
                    ...
                    if inst_analysis.get('patterns'):
                        ...
                        # Add proper color coding considering option type
...
st.caption("🔔 Telegram alerts sent when Technical Analysis and Options Chain biases are aligned")

# Display optimized weights in sidebar
st.sidebar.markdown("**Optimized Bias Weights:**")
st.sidebar.markdown("- Technical Analysis: 60%")
st.sidebar.markdown("- Options Chain: 40%")
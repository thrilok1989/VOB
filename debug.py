...
class NSEOptionsAnalyzer:
        ...
        try:
            ...
            if 'records' not in data:
                return {
                    'success': False,
                    'instrument': instrument,
                    'error': 'Invalid response format from NSE'
                }
    ...
    def analyze_comprehensive_atm_bias(self, instrument: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not data['success']:
                st.warning(f"Could not fetch data for {instrument}: {data.get('error', 'Unknown error')}")
                return None
            ...
            if df_atm.empty:
                st.warning(f"No ATM strikes found for {instrument}")
                return None
            ...
            if not atm_df.empty:
                ...
                st.warning(f"ATM strike data not found for {instrument}")
                return None
            ...
            st.error(f"Error in ATM bias analysis for {instrument}: {str(e)}")
            return None
...
class DataFetcher:
    ...
    def fetch_nifty_data(self, interval: str = "5m", period: str = "5d") -> pd.DataFrame:
        ...
        try:
            ...
            symbols = ["^NSEI", "NSEI", "NIFTY.NS", "^NSEI.NS"]
            ...
            for symbol in symbols:
                try:
                    ...
                    if not df.empty and len(df) > 0:
                        ...
                        st.success(f"Successfully loaded data using {symbol}")
                        return df
                        
                except Exception as e:
                    st.warning(f"Failed with {symbol}: {str(e)}")
                    continue
            
            # If all symbols fail, create sample data for demonstration
            st.warning("Could not fetch live data. Using sample data for demonstration.")
...
class SimplifiedNiftyApp:
    ...
    def display_options_analysis(self):
            ...
            if st.button("🔄 Force Refresh", type="primary"):
                with st.spinner("Force refreshing options data..."):
                    ...
                    if bias_data:
                        ...
                        st.warning("Options data refresh failed. Using cached data if available.")
        with col3:
        ...
        if st.session_state.market_bias_data:
            ...
            st.info("👆 Click 'Force Refresh' to load options data")
            st.write("""
            **What this analysis provides:**
            - Real-time options chain data from NSE
            - ATM bias analysis with multiple indicators  
            - PCR OI and change analysis
            - Synthetic future bias calculation
            - Max pain and key level identification
